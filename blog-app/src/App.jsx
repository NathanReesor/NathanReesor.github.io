      ...
      {tab === "sedar sentiment" && sel === "NXE" && (() => {
        if (sedarLoading) {
          return (
            <Card style={{ marginBottom: 12, textAlign: "center", padding: 30 }}>
              ...
              <div style={{ fontSize: 9, color: C.dm }}>Fetching {sedarUrl}</div>
            </Card>
          );
        }
        if (sedarError) {
          return (
            <Card style={{ marginBottom: 12, textAlign: "center", padding: 30 }}>
              <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 6 }}>Couldn&apos;t load SEDAR sentiment JSON.</div>
              <div style={{ fontSize: 9, color: C.dm, wordBreak: "break-word" }}>{sedarUrl}</div>
              <div style={{ fontSize: 9, color: C.r, marginTop: 6 }}>{sedarError.message}</div>
            </Card>
          );
        }
        if (sedarData.timeline.length === 0) {
          return (
            <Card style={{ marginBottom: 12, textAlign: "center", padding: 30 }}>
              <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 6 }}>SEDAR sentiment data not available.</div>
              <div style={{ fontSize: 9, color: C.dm }}>No timeline data found for {sedarTicker}.</div>
            </Card>
          );
        }
        const DT_LABELS = { interim_mda: "Interim MD&A", annual_mda: "Annual MD&A", aif: "AIF", interim_fs: "Interim FS", annual_fs: "Annual FS", mcr: "Material Change", news_release: "News Release" };
        const dtFilter = sedarDocType === "all_mda" ? ["interim_mda", "annual_mda"] : sedarDocType === "all_fs" ? ["interim_fs", "annual_fs"] : sedarDocType === "all_periodic" ? ["interim_mda","annual_mda","interim_fs","annual_fs"] : sedarDocType === "news_release" ? ["news_release"] : ["interim_mda", "annual_mda"];
        const rawFiltered = sedarData.timeline.filter(t => dtFilter.includes(t.dt)).sort((a,b) => a.p.localeCompare(b.p));

        // For "All Periodic": aggregate multiple filings on same period date into one data point
        const isPeriodicAgg = sedarDocType === "all_periodic" || sedarDocType === "all_mda" || sedarDocType === "all_fs";
        const filtered = (() => {
          if (!isPeriodicAgg) return rawFiltered;
          const byPeriod = {};
          for (const t of rawFiltered) {
            if (!byPeriod[t.p]) byPeriod[t.p] = [];
            byPeriod[t.p].push(t);
          }
          return Object.entries(byPeriod).sort(([a],[b]) => a.localeCompare(b)).map(([p, items]) => {
            if (items.length === 1) return items[0];
            // Average the metrics across filings for this period
            const avg = (arr, key) => arr.reduce((s, t) => s + (t[key] || 0), 0) / arr.length;
            return {
              ...items[0],
              p,
              dt: items.map(i => i.dt).join("+"),
              unc: avg(items, "unc"),
              neg: avg(items, "neg"),
              lit: avg(items, "lit"),
              con: avg(items, "con"),
              ...
              cz: avg(items, "cz"),
              exp: avg(items, "exp"),
              sa: items.reduce((s, t) => s + t.sa, 0),
              sr: items.reduce((s, t) => s + t.sr, 0),
              top: items.map(i => i.top).join(" | "),
              _count: items.length,
            };
          });
        })();
        const metricKey = { composite: "comp", uncertainty: "unc", negative: "neg", litigious: "lit", constraining: "con" }[sedarMetric];
        const zKey = { composite: "cz", uncertainty: "uz", negative: "nz" }[sedarMetric] || "cz";
        const chartData = filtered.map(t => ({ p: t.p.slice(2,7), d: t.metricKey, delta: +(t[metricKey]*1000).toFixed(2), z: +t[zKey].toFixed(2), exp: t.exp, dt: DT_LABELS[t.dt] || t.dt, sa: t.sa, sr: t.sr, top: t.top, period: t.p }));
        const highAlerts = sedarData.alerts.filter(a => dtFilter.includes(a.dt) && Math.abs(a.z) >= 2.0).sort((a,b) => Math.abs(b.z) - Math.abs(a.z));
        const hmFiltered = sedarData.heatmap.filter(h => dtFilter.includes(h.dt));
        // Get unique sections and periods for heatmap
        const hmSections = [...new Set(hmFiltered.map(h => h.sec))].sort();
        const hmPeriods = [...new Set(hmFiltered.map(h => h.p))].sort();
        // Z-score stats
        const zVals = filtered.map(t => t[zKey]).filter(z => z !== null && z !== undefined);
        const outliers = filtered.filter(t => Math.abs(t[zKey]) >= 1.5);
        const latestSpike = outliers.length > 0 ? outliers.reduce((a,b) => a.p > b.p ? a : b) : null;
        return (<>
        <Card style={{ marginBottom: 12, background: C.hi, border: "none" }}>
          <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 4 }}>SEDAR+ Filing Sentiment — NXE Disclosure Language Analysis</div>
          <div style={{ fontSize: 10, color: C.dm, lineHeight: 1.7, maxWidth: 800 }}>
            NLP-based comparison of consecutive SEDAR+ filings. Measures changes in uncertainty, negative, litigious, and constraining language.
            Z-scores computed within each doc type across full filing history. <b style={{color:C.r}}>Red = more risk language added</b>. <b style={{color:C.g}}>Green = risk language removed</b>.
            {(sedarDocType === "news_release" || sedarDocType === "mcr") && (
              <div style={{ marginTop: 4, padding: "4px 8px", background: "#fff8e6", borderLeft: `3px solid ${C.w}`, borderRadius: "0 3px 3px 0", fontSize: 9 }}>
                <b style={{color:C.w}}>⚠ News releases / MCRs:</b> Sequential deltas are suppressed for these doc types — comparing consecutive releases on different topics produces meaningless noise. Use <b>Price × Tone</b> view instead (absolute composite z-scores vs forward returns) or switch to periodic filings (MD&A, FS, AIF) for valid delta analysis.
              </div>
            )}
          </div>
        </Card>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 8, flexWrap: "wrap" }}>
          <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
            <span style={{ fontSize: 8, fontWeight: 700, color: C.dm, fontFamily: "'IBM Plex Mono', monospace" }}>DOC TYPE:</span>
            {[["all_mda","MD&A"],["all_fs","Fin. Stmts"],["all_periodic","All Periodic"],["news_release","News"]].map(([k,l]) => (
              <button key={k} onClick={() => setSedarDocType(k)} style={{ padding: "4px 8px", fontSize: 8, fontWeight: 700, fontFamily: "'IBM Plex Mono', monospace", background: sedarDocType === k ? C.tx : "transparent", color: sedarDocType === k ? "#fff" : C.dm, border: `1px solid ${sedarDocType === k ? C.tx : C.bd}`, borderRadius: 3, cursor: "pointer" }}>{l}</button>
            ))}
          </div>
          <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
            <span style={{ fontSize: 8, fontWeight: 700, color: C.dm, fontFamily: "'IBM Plex Mono', monospace" }}>METRIC:</span>
            {["composite","uncertainty","negative","litigious","constraining"].map(m => (
              <button key={m} onClick={() => setSedarMetric(m)} style={{ padding: "4px 8px", fontSize: 8, fontWeight: 700, fontFamily: "'IBM Plex Mono', monospace", background: sedarMetric === m ? C.bl : "transparent", color: sedarMetric === m ? "#fff" : C.dm, border: `1px solid ${sedarMetric === m ? C.bl : C.bd}`, borderRadius: 3, cursor: "pointer" }}>{m.slice(0,4).toUpperCase()}</button>
            ))}
          </div>
          <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
            <span style={{ fontSize: 8, fontWeight: 700, color: C.dm, fontFamily: "'IBM Plex Mono', monospace" }}>VIEW:</span>
            {[["priceCorr","Price × Tone"],["timeline","Timeline"],["zscores","Z-Scores"],["heatmap","Heatmap"],["alerts","Alerts"]].map(([k,l]) => (
              <button key={k} onClick={() => setSedarView(k)} style={{ padding: "4px 8px", fontSize: 8, fontWeight: 700, fontFamily: "'IBM Plex Mono', monospace", background: sedarView === k ? C.acc : "transparent", color: sedarView === k ? "#fff" : C.dm, border: `1px solid ${sedarView === k ? C.acc : C.bd}`, borderRadius: 3, cursor: "pointer" }}>{l}</button>
            ))}
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 6, marginBottom: 12 }}>
          <Stat l="Filings" v={filtered.length} sub={dtFilter.map(d => DT_LABELS[d]||d).join(", ")} c={C.bl} />
          <Stat l="Date Range" v={`${filtered[0]?.p.slice(0,7) || '—'} → ${filtered[filtered.length-1]?.p.slice(0,7) || '—'}`} sub="filing dates" />
          <Stat l="Best View" v="Price × Tone" sub="Deltas suppressed for this type" c={C.acc} />
        </div>
        ) : (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 6, marginBottom: 12 }}>
          <Stat l="Comparisons" v={filtered.length} sub={dtFilter.map(d => DT_LABELS[d]||d).join(", ")} c={C.bl} />
          <Stat l="Outliers |z|≥1.5" v={outliers.length} sub={`of ${filtered.length} filings`} c={outliers.length > 3 ? C.r : C.g} />
          <Stat l="Latest Spike" v={latestSpike ? `z=${latestSpike[zKey].toFixed(1)}` : "None"} sub={latestSpike ? `${DT_LABELS[latestSpike.dt]} ${latestSpike.p}` : ""} c={latestSpike && latestSpike[zKey] > 0 ? C.r : C.g} />
          <Stat l="Max Risk Increase" v={zVals.length ? `z=+${Math.max(...zVals).toFixed(1)}` : "N/A"} sub={`${sedarMetric} delta`} c={C.r} />
          <Stat l="Max Risk Decrease" v={zVals.length ? `z=${Math.min(...zVals).toFixed(1)}` : "N/A"} sub={`${sedarMetric} delta`} c={C.g} />
        </div>
        )}

        {/* TIMELINE VIEW */}
        {sedarView === "timeline" && (sedarDocType === "news_release" || sedarDocType === "mcr") && (
          <Card style={{ marginBottom: 12, textAlign: "center", padding: 30 }}>
            <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 6 }}>Timeline Deltas — Not Available for News / MCR</div>
            <div style={{ fontSize: 10, color: C.dm, maxWidth: 500, margin: "0 auto", lineHeight: 1.7 }}>
              Sequential deltas between consecutive news releases are meaningless — each release covers a different topic. Switch to <b>MD&A</b>, <b>Fin. Stmts</b>, or <b>All Periodic</b> for valid delta analysis, or use <b>Price × Tone</b> view which works with all doc types.
            </div>
            <div style={{ display: "flex", gap: 8, justifyContent: "center", marginTop: 12 }}>
              <button onClick={() => setSedarView("priceCorr")} style={{ padding: "6px 16px", background: C.acc, color: "#fff", border: "none", borderRadius: 3, fontFamily: "'IBM Plex Mono', monospace", fontSize: 9, fontWeight: 700, cursor: "pointer" }}>Price × Tone →</button>
              <button onClick={() => setSedarDocType("all_mda")} style={{ padding: "6px 16px", background: C.tx, color: "#fff", border: "none", borderRadius: 3, fontFamily: "'IBM Plex Mono', monospace", fontSize: 9, fontWeight: 700, cursor: "pointer" }}>Switch to MD&A</button>
            </div>
          </Card>
        )}
        {sedarView === "timeline" && sedarDocType !== "news_release" && sedarDocType !== "mcr" && (<>
          <Card style={{ marginBottom: 12 }}>
            <Lbl>{sedarMetric} Tone Delta — {dtFilter.map(d=>DT_LABELS[d]).join(" + ")}</Lbl>
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={chartData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#e5e0d8" />
                <XAxis dataKey="p" tick={{ fontSize: 7, fill: C.dm }} interval={Math.max(0, Math.floor(chartData.length/12))} />
                <YAxis tick={{ fontSize: 8, fill: C.dm }} label={{ value: "δ × 1000", angle: -90, position: "insideLeft", style: { fontSize: 8, fill: C.dm } }} />
                <Tooltip contentStyle={tt} formatter={(v,n,p) => {
                  if (n === "Tone Δ (×1000)") return [v.toFixed(2), `${sedarMetric} Δ`];
                  return [v, n];
                }} labelFormatter={(l,p) => p[0] ? `${p[0].payload.dt} — ${p[0].payload.period}` : l} />
                <ReferenceLine y={0} stroke={C.tx} />
                <Bar dataKey="delta" name="Tone Δ (×1000)" radius={[2,2,0,0]}>
                  {chartData.map((d, i) => <Cell key={i} fill={d.delta > 0 ? C.r + (Math.abs(d.z) >= 1.5 ? "dd" : "77") : C.g + (Math.abs(d.z) >= 1.5 ? "dd" : "77")} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
            <div style={{ fontSize: 8, color: C.dm, marginTop: 4 }}>
              Bars show per-filing change in {sedarMetric} language density (×1000 for readability). Saturated bars = outliers (|z| ≥ 1.5). Click Z-Scores tab for detail.
            </div>
          </Card>
          {/* Expansion % sparkline */}
          <Card style={{ marginBottom: 12 }}>
            <Lbl>Document Expansion — {dtFilter.map(d=>DT_LABELS[d]).join(" + ")}</Lbl>
            <ResponsiveContainer width="100%" height={120}>
              <BarChart data={chartData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#e5e0d8" />
                <XAxis dataKey="p" tick={{ fontSize: 7, fill: C.dm }} interval={Math.max(0, Math.floor(chartData.length/12))} />
                <YAxis tick={{ fontSize: 8, fill: C.dm }} label={{ value: "Δ%", angle: -90, position: "insideLeft", style: { fontSize: 8, fill: C.dm } }} />
                <Tooltip contentStyle={tt} labelFormatter={(l,p) => p[0] ? `${p[0].payload.dt} — ${p[0].payload.period}` : l} />
                <ReferenceLine y={0} stroke={C.tx} />
                <Bar dataKey="exp" name="Expansion %" radius={[2,2,0,0]}>
                  {chartData.map((d, i) => <Cell key={i} fill={d.exp > 0 ? C.bl + "88" : C.w + "88"} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
            <div style={{ fontSize: 8, color: C.dm, marginTop: 4 }}>
              Positive = document grew (more sentences added than removed). Large expansion often accompanies new risk disclosures.
            </div>
          </Card>
        </>)}

        {/* Z-SCORES VIEW */}
        {sedarView === "zscores" && (sedarDocType === "news_release" || sedarDocType === "mcr") && (
          <Card style={{ marginBottom: 12, textAlign: "center", padding: 30 }}>
            <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 6 }}>Z-Score Deltas — Not Available for News / MCR</div>
            <div style={{ fontSize: 10, color: C.dm, maxWidth: 500, margin: "0 auto", lineHeight: 1.7 }}>Sequential deltas between news releases are not meaningful. Use <b>Price × Tone</b> or switch to periodic filings.</div>
            <div style={{ display: "flex", gap: 8, justifyContent: "center", marginTop: 12 }}>
              <button onClick={() => setSedarView("priceCorr")} style={{ padding: "6px 16px", background: C.acc, color: "#fff", border: "none", borderRadius: 3, fontFamily: "'IBM Plex Mono', monospace", fontSize: 9, fontWeight: 700, cursor: "pointer" }}>Price × Tone →</button>
              <button onClick={() => setSedarDocType("all_mda")} style={{ padding: "6px 16px", background: C.tx, color: "#fff", border: "none", borderRadius: 3, fontFamily: "'IBM Plex Mono', monospace", fontSize: 9, fontWeight: 700, cursor: "pointer" }}>Switch to MD&A</button>
            </div>
        ...
        {sedarView === "zscores" && sedarDocType !== "news_release" && sedarDocType !== "mcr" && (
          <Card style={{ marginBottom: 12 }}>
            <Lbl>Z-Score Table — {sedarMetric} delta by filing</Lbl>
            <div style={{ maxHeight: 420, overflowY: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 9 }}>
                <thead><tr style={{ borderBottom: `2px solid ${C.tx}`, position: "sticky", top: 0, background: C.card }}>
                  {[["period","Period"],["doctype","Doc Type"],["z",`${sedarMetric.slice(0,4)} z`],["delta","Δ×1000"],["exp","Exp%"],[null,"+Sent"],[null,"−Sent"],[null,"Top Sections"]].map(([sk,h]) => (
                    <th key={h} onClick={sk ? () => setSedarSort(prev => ({ col: sk, asc: prev.col === sk ? !prev.asc : true })) : undefined} style={{ padding: "4px 6px", textAlign: "left", fontWeight: 800, fontSize: 7, fontFamily: "'IBM Plex Mono', monospace", cursor: sk ? "pointer" : "default", userSelect: "none", color: sedarSort.col === sk ? C.acc : undefined }}>
                      {h}{sedarSort.col === sk ? (sedarSort.asc ? " ↑" : " ↓") : ""}
                    </th>
                  ))}
                </tr></thead>
                <tbody>
                  {[...filtered].sort((a,b) => {
                    const dir = sedarSort.asc ? 1 : -1;
                    if (sedarSort.col === "period") return dir * a.p.localeCompare(b.p);
                    if (sedarSort.col === "doctype") return dir * (DT_LABELS[a.dt]||a.dt).localeCompare(DT_LABELS[b.dt]||b.dt);
                    if (sedarSort.col === "z") return dir * (a[zKey] - b[zKey]);
                    if (sedarSort.col === "delta") return dir * (a[metricKey] - b[metricKey]);
                    if (sedarSort.col === "exp") return dir * (a.exp - b.exp);
                    return 0;
                  }).map((t, i) => {
                    const z = t[zKey];
                    const absZ = Math.abs(z);
                    const zBg = absZ >= 2 ? (z > 0 ? "#fde8e3" : "#dff0df") : absZ >= 1.5 ? (z > 0 ? "#fef3f0" : "#edf7ed") : "transparent";
                    return (
                      <tr key={i} style={{ borderBottom: `1px solid ${C.bd}`, background: zBg }}>
                        <td style={{ padding: "4px 6px", fontFamily: "'IBM Plex Mono', monospace", fontWeight: 600 }}>{t.p}</td>
                        <td style={{ padding: "4px 6px", fontSize: 8 }}>{DT_LABELS[t.dt]}</td>
                        <td style={{ padding: "4px 6px", fontWeight: 800, fontFamily: "'IBM Plex Mono', monospace", color: z > 0 ? C.r : C.g }}>
                          {z > 0 ? "+" : ""}{z.toFixed(2)}
                          {absZ >= 2 && <span style={{ marginLeft: 4, padding: "1px 4px", borderRadius: 2, fontSize: 7, background: z > 0 ? C.r : C.g, color: "#fff" }}>!</span>}
                          {absZ >= 1.5 && absZ < 2 && <span style={{ marginLeft: 4, padding: "1px 4px", borderRadius: 2, fontSize: 7, background: C.w, color: "#fff" }}>⚠</span>}
                        </td>
                        <td style={{ padding: "4px 6px", fontFamily: "'IBM Plex Mono', monospace", color: t[metricKey] > 0 ? C.r : C.g }}>{(t[metricKey]*1000).toFixed(2)}</td>
                        <td style={{ padding: "4px 6px", fontFamily: "'IBM Plex Mono', monospace" }}>{t.exp > 0 ? "+" : ""}{t.exp.toFixed(0)}%</td>
                        <td style={{ padding: "4px 6px", color: C.g, fontFamily: "'IBM Plex Mono', monospace" }}>+{t.sa}</td>
                        <td style={{ padding: "4px 6px", color: C.r, fontFamily: "'IBM Plex Mono', monospace" }}>−{t.sr}</td>
                        <td style={{ padding: "4px 6px", fontSize: 7, color: C.dm, maxWidth: 260, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{t.top}</td>
                      </tr>
                    );
                  })}
                </tbody>
            ...
            <div style={{ fontSize: 8, color: C.dm, marginTop: 6 }}>
              Clickable headers to sort. <span style={{ padding: "1px 4px", borderRadius: 2, fontSize: 7, background: C.r, color: "#fff" }}>!</span> = |z| ≥ 2.0 (strong outlier). <span style={{ padding: "1px 4px", borderRadius: 2, fontSize: 7, background: C.w, color: "#fff" }}>⚠</span> = |z| ≥ 1.5 (moderate). Red = more risk language; green = less.
            </div>
          </Card>
        )}

        {/* ALERTS VIEW */}
        {sedarView === "alerts" && (sedarDocType === "news_release" || sedarDocType === "mcr") && (
          <Card style={{ marginBottom: 12, textAlign: "center", padding: 30 }}>
            <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 6 }}>Section Alerts — Not Available for News / MCR</div>
            <div style={{ fontSize: 10, color: C.dm, maxWidth: 500, margin: "0 auto", lineHeight: 1.7 }}>Section-level delta alerts are not meaningful for non-periodic filings. Use <b>Price × Tone</b> or switch to periodic filings.</div>
            <div style={{ display: "flex", gap: 8, justifyContent: "center", marginTop: 12 }}>
              <button onClick={() => setSedarView("priceCorr")} style={{ padding: "6px 16px", background: C.acc, color: "#fff", border: "none", borderRadius: 3, fontFamily: "'IBM Plex Mono', monospace", fontSize: 9, fontWeight: 700, cursor: "pointer" }}>Price × Tone →</button>
              <button onClick={() => setSedarDocType("all_mda")} style={{ padding: "6px 16px", background: C.tx, color: "#fff", border: "none", borderRadius: 3, fontFamily: "'IBM Plex Mono', monospace", fontSize: 9, fontWeight: 700, cursor: "pointer" }}>Switch to MD&A</button>
            </div>
        ...
        {sedarView === "alerts" && sedarDocType !== "news_release" && sedarDocType !== "mcr" && (
          <Card style={{ marginBottom: 12 }}>
            <Lbl>Section-Level Alerts — |z| ≥ 2.0</Lbl>
            {highAlerts.length === 0 ? (
              <div style={{ fontSize: 10, color: C.dm, padding: 20, textAlign: "center" }}>No alerts above threshold for this doc type filter.</div>
            ) : (
              <div style={{ maxHeight: 440, overflowY: "auto" }}>
                {highAlerts.slice(0, 25).map((a, i) => (
                  <div key={i} style={{ display: "grid", gridTemplateColumns: "8px 80px 1fr auto", gap: 8, padding: "8px 6px", borderBottom: `1px solid ${C.bd}`, alignItems: "start" }}>
                    <div style={{ width: 8, height: 8, borderRadius: "50%", marginTop: 3, background: a.dir === "risk_increase" ? C.r : C.g }} />
                    <div>
                      <div style={{ fontSize: 9, fontWeight: 700, fontFamily: "'IBM Plex Mono', monospace" }}>{a.p}</div>
                      <div style={{ fontSize: 7, color: C.dm }}>{DT_LABELS[a.dt]}</div>
                    </div>
                    <div>
                      <div style={{ fontSize: 9, fontWeight: 600 }}>
                        <span style={{ textTransform: "capitalize" }}>{a.sec.replace(/_/g, " ")}</span>
                        <span style={{ marginLeft: 6, fontSize: 7, padding: "1px 5px", borderRadius: 2, background: a.dir === "risk_increase" ? "#fde8e3" : "#dff0df", color: a.dir === "risk_increase" ? C.r : C.g, fontWeight: 700, textTransform: "uppercase" }}>{a.met}</span>
                      </div>
                      <div style={{ fontSize: 8, color: C.dm, marginTop: 2 }}>
                        {a.dir === "risk_increase" ? "↑" : "↓"} {a.sa > 0 ? `+${a.sa} sentences added` : "Sentences removed"}
                      </div>
                    </div>
                    <div style={{ textAlign: "right" }}>
                      <div style={{ fontSize: 12, fontWeight: 800, fontFamily: "'IBM Plex Mono', monospace", color: a.z > 0 ? C.r : C.g }}>{a.z > 0 ? "+" : ""}{a.z.toFixed(1)}σ</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div style={{ fontSize: 8, color: C.dm, marginTop: 6 }}>
              Section-level outliers where a specific section of a filing changed abnormally vs its historical average. These are the most actionable signals — read the actual filing text.
            </div>
          </Card>
        )}

        {/* HEATMAP VIEW */}
        {sedarView === "heatmap" && (sedarDocType === "news_release" || sedarDocType === "mcr") && (
          <Card style={{ marginBottom: 12, textAlign: "center", padding: 30 }}>
            <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 6 }}>Heatmap — Not Available for News / MCR</div>
            <div style={{ fontSize: 10, color: C.dm, maxWidth: 500, margin: "0 auto", lineHeight: 1.7 }}>Section-level heatmaps require periodic filings with consistent structure. Use <b>Price × Tone</b> or switch to periodic filings.</div>
            <div style={{ display: "flex", gap: 8, justifyContent: "center", marginTop: 12 }}>
              <button onClick={() => setSedarView("priceCorr")} style={{ padding: "6px 16px", background: C.acc, color: "#fff", border: "none", borderRadius: 3, fontFamily: "'IBM Plex Mono', monospace", fontSize: 9, fontWeight: 700, cursor: "pointer" }}>Price × Tone →</button>
              <button onClick={() => setSedarDocType("all_mda")} style={{ padding: "6px 16px", background: C.tx, color: "#fff", border: "none", borderRadius: 3, fontFamily: "'IBM Plex Mono', monospace", fontSize: 9, fontWeight: 700, cursor: "pointer" }}>Switch to MD&A</button>
            </div>
        ...
        {sedarView === "heatmap" && sedarDocType !== "news_release" && sedarDocType !== "mcr" && hmPeriods.length > 0 && (
          <Card style={{ marginBottom: 12 }}>
            <Lbl>Section × Period Heatmap — {dtFilter.map(d=>DT_LABELS[d]).join(" + ")} — {sedarMetric}</Lbl>
            <div style={{ overflowX: "auto" }}>
              <table style={{ borderCollapse: "collapse", fontSize: 7, fontFamily: "'IBM Plex Mono', monospace", minWidth: "100%" }}>
                <thead><tr>
                  <th style={{ padding: "4px 6px", textAlign: "left", fontSize: 7, fontWeight: 800, position: "sticky", left: 0, background: C.card, zIndex: 1 }}>Section</th>
                  {hmPeriods.map(p => <th key={p} style={{ padding: "4px 3px", textAlign: "center", fontSize: 6, fontWeight: 700, whiteSpace: "nowrap" }}>{p.slice(2,7)}</th>)}
                </tr></thead>
                <tbody>
                  {hmSections.filter(s => !s.startsWith("_")).map(sec => (
                    <tr key={sec} style={{ borderBottom: `1px solid ${C.bd}` }}>
                      <td style={{ padding: "4px 6px", fontWeight: 600, fontSize: 7, whiteSpace: "nowrap", position: "sticky", left: 0, background: C.card, zIndex: 1 }}>{sec.replace(/_/g, " ")}</td>
                      {hmPeriods.map(p => {
                        const cell = hmFiltered.find(h => h.sec === sec && h.p === p);
                        if (!cell) return <td key={p} style={{ padding: 2, textAlign: "center" }}>—</td>;
                        const v = sedarMetric === "composite" ? cell.comp : cell.unc;
                        const intensity = Math.min(1, Math.abs(v) / 0.02);
                        const bg = v > 0 ? `rgba(194,69,37,${intensity * 0.6})` : `rgba(26,107,58,${intensity * 0.6})`;
                        return <td key={p} style={{ padding: 2, textAlign: "center", background: bg, color: intensity > 0.4 ? "#fff" : C.tx, fontSize: 7, fontWeight: intensity > 0.3 ? 700 : 400 }}>
                          {(v * 1000).toFixed(0)}
                        </td>;
                      })}
                    </tr>
            ...
            <div style={{ fontSize: 8, color: C.dm, marginTop: 6 }}>
              Values = {sedarMetric} Δ × 1000. <span style={{ color: C.r }}>Red = more risk language added</span>, <span style={{ color: C.g }}>green = removed</span>. Intensity = magnitude. Tracks which sections are getting hotter or cooler over time.
            </div>
          </Card>
        )}
        ...
        {sedarView === "priceCorr" && (() => {
          ...
          const findPrice = (date) => {
            ...
            for (const p of prices) {
              const dist = Math.abs(new Date(p.d) - new Date(date));
              if (dist < bestDist) { bestDist = dist; best = p; }
            }
            return best;
          };

          // Helper: find price N weeks after a date
          const findPriceAfter = (date, weeks) => {
            const target = new Date(date);
            target.setDate(target.getDate() + weeks * 7);
            return findPrice(target.toISOString().slice(0, 10));
          };

          // Build correlation dataset
          const windows = [2, 4, 8, 13, 26]; // weeks forward
          const corrDataset = toneFilings.map(f => {
            // News/MCR: available immediately on filing date
            // Periodic: ~45 day SEDAR lag after period-end
            let filed;
            if (eventTypes.includes(f.dt)) {
              filed = f.p;
            } else {
              const periodDate = new Date(f.p);
              const estFiledDate = new Date(periodDate);
              estFiledDate.setDate(estFiledDate.getDate() + 45);
              filed = estFiledDate.toISOString().slice(0, 10);
            }
            
            const basePrice = findPrice(filed);
            ...
            const row = {
              ...
              baseDate: basePrice.d,
            };

            for (const w of windows) {
              const fwdPrice = findPriceAfter(filed, w);
              if (fwdPrice) {
                row[`r${w}`] = +((fwdPrice.c / basePrice.c - 1) * 100).toFixed(2);
                row[`p${w}`] = fwdPrice.c;
              }
            }
            return row;
          }).filter(Boolean).filter(r => r.r2 !== undefined);

          // Compute Pearson correlation
          const pearson = (xs, ys) => {
            const n = xs.length;
            if (n < 4) return { r: 0, p: 1, n };
            ...
            const df = n - 2;
            const p_approx = Math.exp(-0.717 * Math.abs(t) - 0.416 * t * t / df);
            return { r: +r.toFixed(3), p: +Math.min(1, p_approx).toFixed(4), n };
          };

          // Compute correlations for each metric × each window
          const metrics = [
            { key: "comp", zKey: "cz", label: "Composite Δ", zLabel: "Composite Z" },
            { key: "unc", zKey: "uz", label: "Uncertainty Δ", zLabel: "Uncertainty Z" },
            { key: "neg", zKey: "nz", label: "Negative Δ", zLabel: "Negative Z" },
          ];

          const corrResults = [];
          for (const m of metrics) {
            for (const w of windows) {
              const valid = corrDataset.filter(r => r[`r${w}`] !== undefined && r[m.key] !== undefined);
              const xs = valid.map(r => r[m.key] * 1000); // scale for readability
              const ys = valid.map(r => r[`r${w}`]);
              const { r, p, n } = pearson(xs, ys);
              corrResults.push({ metric: m.label, window: `+${w}w`, weeks: w, r, p, n, sig: p < 0.1 });

              // Also z-score correlation
              const xz = valid.map(r => r[m.zKey]);
              const yz = valid.map(r => r[`r${w}`]);
              const zCorr = pearson(xz, yz);
              corrResults.push({ metric: m.zLabel, window: `+${w}w`, weeks: w, r: zCorr.r, p: zCorr.p, n: zCorr.n, sig: zCorr.p < 0.1 });
            }
          }

          // Chart data: scatter plot of composite z vs forward returns
          const scatterWindow = 13; // 13 weeks = 1 quarter forward
          const scatterData = corrDataset
            .filter(r => r[`r${scatterWindow}`] !== undefined)
            .map(r => ({ name: r.period, x: r.cz, y: r[`r${scatterWindow}`], dt: r.dt === "annual_mda" ? "Annual" : "Interim" }));

          // For the overlay chart: tone timeline with price
          const overlayData = toneFilings.map(f => {
            const pp = findPrice(f.p);
            return { p: f.p.slice(2,7), tone: +(f[metricKey]*1000).toFixed(2), z: f[zKey], price: pp ? pp.c : null, period: f.p, dt: DT_LABELS[f.dt] };
          });

          return (<>
          <Card style={{ marginBottom: 12 }}>
            <Lbl>NXE Price Overlay — {sedarMetric} Tone Delta vs Weekly Close</Lbl>
            <ResponsiveContainer width="100%" height={240}>
              <ComposedChart data={overlayData} margin={{ top: 5, right: 40, left: 0, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#e5e0d8" />
                <XAxis dataKey="p" tick={{ fontSize: 7, fill: C.dm }} />
                <YAxis yAxisId="tone" tick={{ fontSize: 8, fill: C.dm }} label={{ value: "δ×1000", angle: -90, position: "insideLeft", style: { fontSize: 8, fill: C.dm } }} />
                <YAxis yAxisId="price" orientation="right" tick={{ fontSize: 8, fill: C.bl }} label={{ value: "C$", angle: 90, position: "insideRight", style: { fontSize: 8, fill: C.bl } }} />
                <Tooltip contentStyle={tt} labelFormatter={(l,p) => p[0] ? `${p[0].payload.dt} — ${p[0].payload.period}` : l} />
                <ReferenceLine yAxisId="tone" y={0} stroke={C.tx} strokeDasharray="3 3" />
                <Bar yAxisId="tone" dataKey="tone" name="Tone Δ (×1000)" radius={[2,2,0,0]}>
                  {overlayData.map((d, i) => <Cell key={i} fill={d.tone > 0 ? C.r + "88" : C.g + "88"} />)}
                </Bar>
                <Line yAxisId="price" type="monotone" dataKey="price" name="NXE Close (C$)" stroke={C.bl} strokeWidth={2} dot={{ r: 3, fill: C.bl }} connectNulls />
              </ComposedChart>
            </ResponsiveContainer>
            <div style={{ fontSize: 8, color: C.dm, marginTop: 4 }}>
          ...
          {/* Correlation matrix */}
          <Card style={{ marginBottom: 12 }}>
            <Lbl>Tone → Forward Return Correlation ({dtFilter.map(d=>DT_LABELS[d]).join(" + ")} × NXE Price)</Lbl>
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 9 }}>
                <thead><tr style={{ borderBottom: `2px solid ${C.tx}` }}>
                  <th style={{ padding: "4px 8px", textAlign: "left", fontWeight: 800, fontSize: 7, fontFamily: "'IBM Plex Mono', monospace" }}>Tone Metric</th>
                  {windows.map(w => <th key={w} style={{ padding: "4px 8px", textAlign: "center", fontWeight: 800, fontSize: 7, fontFamily: "'IBM Plex Mono', monospace" }}>+{w}w Return</th>)}
                </tr></thead>
                <tbody>
                  {metrics.flatMap(m => [m.label, m.zLabel]).map(mLabel => {
                    const rows = corrResults.filter(r => r.metric === mLabel);
                    return (
                      <tr key={mLabel} style={{ borderBottom: `1px solid ${C.bd}`, background: mLabel.includes("Z") ? C.hi : "transparent" }}>
                        <td style={{ padding: "5px 8px", fontWeight: 600, fontSize: 8, fontFamily: "'IBM Plex Mono', monospace" }}>{mLabel}</td>
                        {windows.map(w => {
                          const cr = rows.find(r => r.weeks === w);
                          if (!cr) return <td key={w} style={{ padding: "5px 8px", textAlign: "center" }}>—</td>;
                          const absR = Math.abs(cr.r);
                          const color = cr.r < 0 ? C.g : C.r; // negative r = more risk language → lower returns (expected)
                          const bgIntensity = absR > 0.3 ? 0.15 : absR > 0.2 ? 0.08 : 0;
                          return (
                            <td key={w} style={{ padding: "5px 8px", textAlign: "center", fontFamily: "'IBM Plex Mono', monospace", fontWeight: cr.sig ? 800 : 400, color: absR > 0.15 ? color : C.dm, background: bgIntensity > 0 ? (cr.r < 0 ? `rgba(26,107,58,${bgIntensity})` : `rgba(194,69,37,${bgIntensity})`) : "transparent" }}>
                              {cr.r > 0 ? "+" : ""}{cr.r.toFixed(2)}
                              {cr.sig && <span style={{ fontSize: 6, verticalAlign: "super" }}>*</span>}
                              <div style={{ fontSize: 6, color: C.dm }}>n={cr.n}</div>
                            </td>
                          );
                        })}
                      </tr>
            ...
            <div style={{ fontSize: 8, color: C.dm, marginTop: 6 }}>
              Pearson r: tone metric vs NXE forward return at each window. <span style={{ fontSize: 6, verticalAlign: "super" }}>*</span> = p &lt; 0.10.
              <b> Negative r for risk metrics is the expected direction</b> — more risk language → lower forward returns.
              Filing lag of ~45 days assumed (period-end → SEDAR availability).
            </div>
          </Card>

          {/* Scatter plot */}
          <Card style={{ marginBottom: 12 }}>
            <Lbl>Scatter: Composite Z-Score vs +13w Forward Return</Lbl>
            <ResponsiveContainer width="100%" height={220}>
              <ComposedChart data={scatterData} margin={{ top: 10, right: 20, left: 0, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#e5e0d8" />
                <XAxis dataKey="x" type="number" tick={{ fontSize: 8, fill: C.dm }} label={{ value: "Composite Z", position: "insideBottom", offset: -2, style: { fontSize: 8, fill: C.dm } }} />
                <YAxis tick={{ fontSize: 8, fill: C.dm }} label={{ value: "+13w Return %", angle: -90, position: "insideLeft", style: { fontSize: 8, fill: C.dm } }} />
                <Tooltip contentStyle={tt} formatter={(v, n) => [typeof v === "number" ? v.toFixed(2) : v, n]} labelFormatter={(l, p) => p[0] ? p[0].payload.name : ""} />
                <ReferenceLine x={0} stroke={C.dm} strokeDasharray="3 3" />
                <ReferenceLine y={0} stroke={C.dm} strokeDasharray="3 3" />
                {scatterData.map((d, i) => (
                  <ReferenceLine key={`dot-${i}`} x={null} y={null} label="" />
                ))}
                <Line type="monotone" dataKey="y" stroke="none" dot={{ r: 5, fill: C.acc, stroke: C.acc, strokeWidth: 1 }} isAnimationActive={false} />
              </ComposedChart>
            </ResponsiveContainer>
            <div style={{ display: "flex", gap: 12, marginTop: 4 }}>
              {scatterData.map((d, i) => (
                <div key={i} style={{ fontSize: 6, color: C.dm, fontFamily: "'IBM Plex Mono', monospace" }}>
                  {d.name}: z={d.x.toFixed(1)} → {d.y > 0 ? "+" : ""}{d.y.toFixed(0)}%
                </div>
              ))}
            </div>
            <div style={{ fontSize: 8, color: C.dm, marginTop: 6 }}>
              Each dot = one filing. X = composite z-score, Y = NXE return over next 13 weeks.
              If tone changes predict price, expect downward slope (negative correlation).
            </div>
          </Card>

          {/* Event study */}
          <Card style={{ marginBottom: 12 }}>
            <Lbl>Event Study — +13w Returns by Tone Signal ({dtFilter.map(d=>DT_LABELS[d]).join(" + ")})</Lbl>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <div>
                <div style={{ fontSize: 9, fontWeight: 700, color: C.r, marginBottom: 4 }}>↑ RISK INCREASE (z &gt; 0)</div>
                {corrDataset.filter(r => r.cz > 0 && r.r13 !== undefined).sort((a,b) => b.cz - a.cz).map((r, i) => (
                  <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "3px 0", borderBottom: `1px solid ${C.bd}`, fontSize: 8 }}>
                    <span style={{ fontFamily: "'IBM Plex Mono', monospace" }}>{r.period}</span>
                    <span style={{ color: C.r, fontWeight: 600 }}>z={r.cz.toFixed(1)}</span>
                    <span style={{ fontWeight: 700, color: r.r13 < 0 ? C.r : C.g }}>{r.r13 > 0 ? "+" : ""}{r.r13.toFixed(1)}%</span>
                  </div>
                ))}
                {(() => {
                  const up = corrDataset.filter(r => r.cz > 0 && r.r13 !== undefined);
                  const avg = up.length ? (up.reduce((a,r) => a + r.r13, 0) / up.length).toFixed(1) : "N/A";
                  return <div style={{ padding: "5px 0", fontWeight: 800, fontSize: 9, borderTop: `2px solid ${C.tx}`, marginTop: 2 }}>Avg +13w return: <span style={{ color: +avg < 0 ? C.r : C.g }}>{avg}%</span> (n={up.length})</div>;
                })()}
              </div>
              <div>
                <div style={{ fontSize: 9, fontWeight: 700, color: C.g, marginBottom: 4 }}>↓ RISK DECREASE (z &lt; 0)</div>
                {corrDataset.filter(r => r.cz < 0 && r.r13 !== undefined).sort((a,b) => a.cz - b.cz).map((r, i) => (
                  <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "3px 0", borderBottom: `1px solid ${C.bd}`, fontSize: 8 }}>
                    <span style={{ fontFamily: "'IBM Plex Mono', monospace" }}>{r.period}</span>
                    <span style={{ color: C.g, fontWeight: 600 }}>z={r.cz.toFixed(1)}</span>
                    <span style={{ fontWeight: 700, color: r.r13 < 0 ? C.r : C.g }}>{r.r13 > 0 ? "+" : ""}{r.r13.toFixed(1)}%</span>
                  </div>
                ))}
                {(() => {
                  const dn = corrDataset.filter(r => r.cz < 0 && r.r13 !== undefined);
                  const avg = dn.length ? (dn.reduce((a,r) => a + r.r13, 0) / dn.length).toFixed(1) : "N/A";
                  return <div style={{ padding: "5px 0", fontWeight: 800, fontSize: 9, borderTop: `2px solid ${C.tx}`, marginTop: 2 }}>Avg +13w return: <span style={{ color: +avg < 0 ? C.r : C.g }}>{avg}%</span> (n={dn.length})</div>;
                })()}
              </div>
            </div>
            <div style={{ padding: 8, marginTop: 8, borderLeft: `3px solid ${C.bl}`, background: "#f0f4f8", borderRadius: "0 3px 3px 0", fontSize: 9, color: C.dm, lineHeight: 1.7 }}>
              <b style={{ color: C.bl }}>METHODOLOGY:</b> Forward returns measured from estimated availability date.
              Periodic filings use +45 day SEDAR lag; news releases and material changes use filing date directly.
              This tests whether an investor reading on publication date could trade profitably on tone changes.
              Sample size varies by doc type filter — use n values in the correlation matrix to assess reliability.
            </div>
          </Card>
          </>);
        })()}
        ...
        {/* Interpretation */}
        <Card>
          <Lbl>Reading the Signals</Lbl>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div style={{ fontSize: 9, lineHeight: 1.8, color: C.dm }}>
              <b style={{ color: C.tx }}>What z-scores mean:</b> A z-score of +1.9 means that filing added 1.9 standard deviations more risk language than NXE's historical average for that doc type. Scores above ±1.5 are unusual; above ±2.0 are statistically significant anomalies.
            </div>
            <div style={{ fontSize: 9, lineHeight: 1.8, color: C.dm }}>
              <b style={{ color: C.tx }}>Composite score:</b> Weighted blend of four Loughran-McDonald tone metrics (40% uncertainty, 30% negative, 15% litigious, 15% constraining). Positive = net increase in risk-oriented disclosure language. Use alerts to find exact sections that changed.
            </div>
          </div>
          <div style={{ marginTop: 10, padding: 8, borderLeft: `3px solid ${C.acc}`, background: "#fdf5f3", borderRadius: "0 3px 3px 0", fontSize: 9, color: C.dm, lineHeight: 1.7 }}>
            <b style={{ color: C.acc }}>KEY FINDING:</b> NXE's Q1 2024 Interim MD&A (period 2024-03-31) shows the strongest risk signal: composite z = +1.9, uncertainty z = +1.9.
            The Q1 2025 MD&A (2025-03-31) also spikes at neg z = +1.7 with major new sections appearing in results_of_operations and risks_uncertainties.
            These coincide with Rook I licensing preparations and the C$800M raise — management materially expanded disclosure around project execution and financing risks.
          </div>
        </Card>
      </>); })()}
      {tab === "sedar sentiment" && sel !== "NXE" && (
