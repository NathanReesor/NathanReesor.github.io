...
const CSV_URL = "/data/spx_weekly.csv"; // UPDATE per codex-prompt.md
...
function SVGBar({data,w:W=480,h:H=210,title}){
  ...
  const x=d3.scaleBand().domain(data.map(d=>d.l)).range([0,w]).padding(.22);
  const mx=Math.max(...data.map(d=>Math.abs(d.v)),.001);
  const y=d3.scaleLinear().domain([-mx*1.3,mx*1.3]).range([h,0]);
  return(
...
function SVGDist({data,w:W=480,h:H=190,title,color=K.blu}){
  ...
  const bins=d3.bin().thresholds(16)(data);
  const x=d3.scaleLinear().domain([bins[0].x0,bins[bins.length-1].x1]).range([0,w]);
...
function SVGHeat({entries,field="opexReturn",w:W=680,title}){
  ...
  const cs=d3.scaleLinear().domain([-mx,0,mx]).range([K.red,K.bg,K.grn]);
  return(
